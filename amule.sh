...
if grep -q ":${AMULE_GID}:" /etc/group; then
    ...
    echo "Create group ${AMULE_GROUP} with ID ${AMULE_GID}"
    addgroup --system --gid "${AMULE_GID}" ${AMULE_GROUP}
fi

if grep -q ":${AMULE_UID}:" /etc/passwd; then
    echo "User ${AMULE_UID} already exists. Won't be added."
else
    echo "Create user 'amule' with ID ${AMULE_UID}"
    adduser --system --shell /sbin/nologin --uid "${AMULE_UID}" --home "/home/amule" --gid "${AMULE_GID}" amule
fi
